 ...
 * HTTPS.
 * <p>
 * A HttpsServer must have an associated {@link HttpsConfigurator} object which
 * is used to establish the SSL configuration for the SSL connections.
 * <p>
...
public abstract class HttpsServer extends HttpServer
{
	...
	/**
	 * creates a HttpsServer instance which is initially not bound to any local
	 * address/port. The HttpsServer is acquired from the currently installed
	 * {@link HttpServerProvider} The server must be bound using
	 * {@link #bind(InetSocketAddress,int)} before it can be used. The server
	 * must also have a HttpsConfigurator established with
	 ...
	 * connections exceeding this limit may be rejected by the TCP
	 * implementation. The HttpsServer is acquired from the currently installed
	 * {@link HttpServerProvider} The server must have a HttpsConfigurator
	 * established with {@link #setHttpsConfigurator(HttpsConfigurator)}
	 * 
	 * @param addr
	 *            the address to listen on, if <code>null</code> then bind()
	 ...
	 * 
	 * @return the HttpsConfigurator for this server, or <code>null</code> if
	 *         not set.
	 */
